...
import { ChevronLeft, ChevronRight, X, ZoomIn, Heart, ShoppingCart, Check, Package, Truck, Shield, Filter, ChevronDown, MoreHorizontal } from 'lucide-react';
import { cricketBatsBrandsNew, cricketBatsNew, getCricketBatsByBrand, batCategories } from '../data/cricketBatsDataNew';
      ...
      >
        ...
        <div className="grid md:grid-cols-2 gap-6 p-6">
          ...
          <div className="space-y-4">
            <div className="relative aspect-[4/5] bg-gray-50 rounded-xl overflow-hidden group">
              <img
          ...
          <div className="space-y-5">
            <div>
              ...
              {product.playerEdition && (
                <p className="text-gray-500 text-sm mt-1">Player Edition: {product.playerEdition}</p>
              )}
            </div>
            ...
            <div className="bg-gray-50 rounded-xl p-4">
              <h4 className="font-bold text-gray-900 mb-2 text-sm">Specifications</h4>
              <div className="space-y-2 text-sm">
                <div className="flex justify-between">
                  <span className="text-gray-500">Willow Type:</span>
                  <span className="font-medium text-gray-900">{product.willow}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-500">Size:</span>
                  <span className="font-medium text-gray-900">Short Handle (SH)</span>
                </div>
              </div>
            </div>
      ...
      <div className="relative aspect-[4/5] overflow-hidden bg-gray-50">
        <img
          ...
          className={`w-full h-full object-cover transition-all duration-700 ${
            !bat.inStock ? 'grayscale opacity-80' : ''
        ...
        {/* Image Navigation */}
        {isHovered && bat.images.length > 1 && (
          <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex gap-1.5 z-10">
            {bat.images.map((_, idx) => (
              <button
                key={idx}
      ...
      <div className="p-4">
        ...
        {bat.reviews > 0 && (
          <div className="mt-2 text-xs text-gray-500">
            {bat.reviews} review{bat.reviews > 1 ? 's' : ''}
          </div>
        )}
      </div>
    </div>
...
const CricketBatsCollectionNew = () => {
  ...
  // Filter and sort
  const filteredAndSortedBats = useMemo(() => {
    let bats = getCricketBatsByBrand(selectedBrand);
    
    if (availabilityFilter === 'in-stock') {
      bats = bats.filter(bat => bat.inStock);
    ...
    switch (sortBy) {
      case 'price-low':
        bats = [...bats].sort((a, b) => a.price - b.price);
        break;
      case 'price-high':
        bats = [...bats].sort((a, b) => b.price - a.price);
        break;
      case 'name-az':
        bats = [...bats].sort((a, b) => a.name.localeCompare(b.name));
        break;
      case 'name-za':
        bats = [...bats].sort((a, b) => b.name.localeCompare(a.name));
        break;
      default:
        break;
    }
  ...
  const totalPages = Math.ceil(filteredAndSortedBats.length / ITEMS_PER_PAGE);
  const paginatedBats = filteredAndSortedBats.slice(
  ...
  const inStockCount = cricketBatsNew.filter(b => b.inStock).length;
  const outOfStockCount = cricketBatsNew.filter(b => !b.inStock).length;
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="relative h-[300px] md:h-[400px] overflow-hidden">
        <img
          src="https://crickstore.com/cdn/shop/collections/IMG-20240829-WA0000.jpg?v=1724870115&width=3072"
          alt="Cricket Bats Collection"
        ...
        <div className="max-w-7xl mx-auto px-4">
          <div className="flex items-center justify-between py-4">
            ...
            <div className="flex items-center gap-2 overflow-x-auto scrollbar-hide flex-1 mr-4">
              {cricketBatsBrandsNew.slice(0, 8).map((brand) => (
                <button
                  key={brand.id}
                  onClick={() => {
        ...
        <div className="bg-white border-b border-gray-200">
          <div className="max-w-7xl mx-auto px-4 py-4">
            <div className="flex flex-wrap gap-6">
              ...
              {/* More Brands */}
              <div>
                <h4 className="font-semibold text-gray-900 mb-2 text-sm">More Brands</h4>
                <div className="flex flex-wrap gap-2">
                  {cricketBatsBrandsNew.slice(8).map((brand) => (
                    <button
                      key={brand.id}
                      onClick={() => {
                        setSelectedBrand(brand.id);
                      ...
                      className={`px-3 py-1.5 rounded-full text-sm transition-all ${
                        selectedBrand === brand.id ? 'bg-emerald-500 text-white' : 'bg-gray-100 text-gray-600'
                      }`}
                    >
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        <div className="flex items-center justify-between mb-6">
          <p className="text-gray-600">
            <span className="font-semibold text-gray-900">{filteredAndSortedBats.length}</span> products
          </p>
...
export default CricketBatsCollectionNew;
