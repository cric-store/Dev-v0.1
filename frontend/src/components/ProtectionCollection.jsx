...
import { protectionCategories, protectionProducts, getProtectionByCategory } from '../data/protectionData';
...
const ProtectionCollection = () => {
  ...
  const filteredProducts = useMemo(() => {
    let products = getProtectionByCategory(selectedCategory);
    ...
    return products;
  }, [selectedCategory, sortBy, availabilityFilter]);

  const inStockCount = protectionProducts.filter(p => p.inStock).length;
  const outOfStockCount = protectionProducts.filter(p => !p.inStock).length;
        ...
        <div className="absolute inset-0 flex items-center">
          <div className="max-w-7xl mx-auto px-4 w-full">
            ...
            <p className="text-gray-300 text-lg max-w-xl">
              Essential cricket body protection gear for safety on the field
            </p>
          </div>
        </div>
      </div>
...
export default ProtectionCollection;
