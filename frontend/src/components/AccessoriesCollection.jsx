...
import { accessoriesBrands, accessories, getAccessoriesByBrand } from '../data/accessoriesData';
...
const AccessoriesCollection = () => {
  ...
  const filteredProducts = useMemo(() => {
    let products = getAccessoriesByBrand(selectedBrand);
  ...
  const inStockCount = accessories.filter(p => p.inStock).length;
  const outOfStockCount = accessories.filter(p => !p.inStock).length;
        ...
        <div className="absolute inset-0 flex items-center">
          <div className="max-w-7xl mx-auto px-4 w-full">
            ...
            <p className="text-gray-300 text-lg max-w-xl">
              Cricket accessories, bat care, grips, sunglasses and training equipment
            </p>
          </div>
        </div>
      </div>
